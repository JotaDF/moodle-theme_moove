...
class certificates {
     ...
     * @throws \moodle_exception
     */
    public function get_from_simplecertificate_old() {
        global $DB;
    ...
    /**
     * Get all certificates from simplecertificate module.
     *
     * @return array
     *
     * @throws \dml_exception
    ...
    public function get_from_simplecertificate() {
        ...
        if ($mycourses = enrol_get_users_courses($this->user->id, true)) {
            $returndata = [];
            foreach ($mycourses as $mycourse) {
                
                $context = \context_course::instance($mycourse->id, MUST_EXIST);
                if (!has_capability('moodle/course:viewhiddenactivities', $context, $this->user->id)) {
                    $modinfo = get_fast_modinfo($mycourse);
                    $certificates = $modinfo->get_instances_of('simplecertificate');
                    foreach ($certificates as $certificate) {
                        if ($certificate->uservisible) {
                            // Disponível.
                            $sql = "SELECT
                                sci.code,
                                sci.pathnamehash,
                            ...
                            FROM {simplecertificate_issues} sci
                            INNER JOIN {simplecertificate} sc ON sc.id = sci.certificateid
                            WHERE sci.timedeleted IS NULL 
                            AND sc.id = :id 
                            AND sci.userid = :userid";
                            
                            $params['id'] = $certificate->instance;
                            $params['userid'] = $this->user->id;
                            
                            $certificatex = $DB->get_record_sql($sql, $params);
                            
                            if (empty($certificatex)) {
                                // Caso o simplecertificate_issues ainda não tenha sido criada 
                            ...
                            WHERE sc.id = :id";
                                ...
                                if (empty($certificatex)) {
                                    ...
                                    $certificatex->pathnamehash = '';
                                }
                            }
                            
                            $certificatex->courseid = $mycourse->id;
                            $certificatex->fullname = $mycourse->fullname;
                            $certificatex->shortname = $mycourse->shortname;

                            $fs = get_file_storage();

                            if ($fs->file_exists_by_hash($certificatex->pathnamehash)) {
                                // Disponível para download.
                                $url = new \moodle_url('/mod/simplecertificate/wmsendfile.php', [
                                    'code' => $certificatex->code
                                ]);

                                $certificatex->downloadurl = $url->out(false);

                                $returndata[] = $certificatex;
                            } else {
                                // Disponível mas não gerada.
                                $url = new \moodle_url('/mod/simplecertificate/view.php', [
                                    'id' => $certificate->id
                                ]);
                                $certificatex->downloadurl = $url->out(false);
                                $certificatex->module = 'simplecertificate';
